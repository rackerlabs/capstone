...
class TestCacheInvalidator(test_integration.BaseIntegrationTests):
    ...
    These test will not work against a real deployment since test were create
    to work with the v2 mock service and auto generated feed events.
    """
    ...
    def _build_content(self, product, resource_id, type):
        return {
            "content": {
                "event": {
    ...
    def _get_delete_token_event(self, token_id):
        ...
        feed['feed']['entry'].append(self._build_content(product,
                                                         token_id,
                                                         "DELETE"))
    ...
    def _return_valid_response(self, content):
        """Build valid HTTP GET response."""
    ...
    def test_revoke_token_event_invalidation(self, m):
        ...
        data = test_integration.generate_password_auth_data({
            "name": uncached_user_id,
            "password": uncached_password,
        })
        resp = self.authenticate(data)
        token2 = resp.headers['X-Subject-Token']
        # Assert tokens are equal once the user and authentication have been
        # cached.
        self.assertEqual(token, token2)

        # Mock CloudFeeds response
        event = self._get_delete_token_event(token)
        m.return_value = self._return_valid_response(event)
    ...
    def _test_cache_invalidation(self, fake_get, user_id, event):
        uncached_password = hashlib.sha1(user_id).hexdigest()
        data = test_integration.generate_password_auth_data({
            "id": user_id,
            "password": uncached_password,
        })
        ...
        # Mock CloudFeeds response
        fake_get.return_value = self._return_valid_response(event)
        entry = cache_invalidator.Entry(self.client.entries[0])
        entry.invalidate()

        # Since the user did not actually get modified, we can authenticate
    ...
    @mock.patch('capstone.cache_invalidator.requests.Session.get')
    def test_disabled_user_invalidation(self, m):
        user_id = uuid.uuid4().hex
        event = self._get_user_event(user_id, 'SUSPEND', 'USER')
        self._test_cache_invalidation(m, user_id, event)

    @mock.patch('capstone.cache_invalidator.requests.Session.get')
    def test_updated_user_invalidation(self, m):
        user_id = uuid.uuid4().hex
        event = self._get_user_event(user_id, 'UPDATE', 'USER')
        self._test_cache_invalidation(m, user_id, event)

    @mock.patch('capstone.cache_invalidator.requests.Session.get')
    def test_deleted_user_invalidation(self, m):
        user_id = uuid.uuid4().hex
        event = self._get_user_event(user_id, 'DELETE', 'USER')
        self._test_cache_invalidation(m, user_id, event)
