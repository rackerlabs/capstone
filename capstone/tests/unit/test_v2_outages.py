...
class TestV2Outages(testtools.TestCase):
    ...
    def test_v2_authentication_with_intermittent_failures(self, m):
        identity = auth_plugin.RackspaceIdentity.from_username(
            self.username, self.password, user_domain_id=self.domain_id)

        user_url = identity.url('/users?name=%s' % self.username)
        m.register_uri('GET', user_url, status_code=200,
                       json=self._get_user())
        m.register_uri('POST', identity.url('/tokens'),
                       [{'status_code': 200, 'json': self._get_token_data()},
                        {'status_code': 502},
                        {'status_code': 200, 'json': self._get_token_data()},
                        {'status_code': 408}])
