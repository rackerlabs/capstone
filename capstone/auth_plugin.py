...
class RackspaceIdentity(object):
    ...
    def handle_unexpected_response(self, resp):
        if resp.status_code == requests.codes.not_found:
            ...
            # Does every exception have it's own response schema? If so, what
            # is the schema of a 401? Where would the message be?
            msg = resp.text
            LOG.info(msg)
    ...
    def get_user(self, user_id):
        ...
        LOG.info(_LI('Found user %s in v2.'), user['name'])
        return user
