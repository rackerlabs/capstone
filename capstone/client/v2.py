...
class RackspaceIdentity(object):
    ...
    def _is_project_domain(self, token_data, project_domain):
        # Ignore project domain if project id is specified.
        if self._scope_project_id:
            return True
        # If the project name matches the domain provided, then its save to
    ...
    def _assert_project_scope(self, token_data):
        ...
        if not self._is_project_domain(token_data, project_domain):
    ...
    def get_project_by_name(self, project_name, params):
        token = self._token_data['access']['token']['id']
        return self.GET('/tenants', params=params, auth_token=token)['tenant']
